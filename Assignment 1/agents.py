...
def TableDrivenAgentProgram(table):
    ...
    """
    percepts = []
    visited_percepts = set()  # Keep track of visited percept sequences to detect cycles
    
    def program(percept):
        percepts.append(percept)
        
        # Convert percepts to a tuple
        percept_tuple = tuple(percepts)
        
        # Check for edge cases, especially when only one dirty cell is left
        if len(percepts) >= 2:
            # Look for edge cases with one dirty cell left
            # E.g., last percepts indicating one dirty cell left
            for i in range(len(percepts) - 1):
                state = tuple(percepts[i:])
                if state in table:
                    return table[state]
        
        # Try to find the longest matching percept sequence
        for i in range(len(percepts), 0, -1):
            action = table.get(tuple(percepts[-i:]))
            if action:
                # Add the current percept sequence to visited percepts
                visited_percepts.add(tuple(percepts[-i:]))
                return action
        
        # Detect cycles: if the current percept sequence was seen before, it means we're in a loop
        if tuple(percepts) in visited_percepts:
            # Handling the loop situation
            return 'NoOp'
        
        # Return a default action if no match is found
        return 'NoOp'
...
# simple reflex agent - edited to handle a 2x2 env
...
# model-based reflex agent - edited for a 2x2 environment
def ModelBasedReflexAgentProgram(rules, update_state, model):
    
    locations = [(0, 0), (1, 0), (0, 1), (1, 1)]  # 2x2 locs
    ...
    program.state = {loc: None for loc in locations}  
    program.action = None
    return program
...
# Edited here for Assignment 1 to accomodate a 2x2 environment
# Define the locations for the 2x2 environment 
loc_A, loc_B, loc_C, loc_D = (0, 0), (1, 0), (0, 1), (1, 1)  

# random agent - edited to handle a 2x2 env
def RandomVacuumAgent():
    def program(percept):
        return random.choice(['Right', 'Left', 'Up', 'Down', 'Suck', 'NoOp'])
    return Agent(program)


# table-driven agent - edited to handle a 2x2 env
# def TableDrivenVacuumAgent():
#     """A vacuum agent that uses a table to decide actions based on percept sequences."""
#     table = {
#      # Single-step sequences
#     ((loc_A, 'Clean'),): 'Right',
#     ((loc_A, 'Dirty'),): 'Suck',
#     ((loc_B, 'Clean'),): 'Down',
#     ((loc_B, 'Dirty'),): 'Suck',  # Agent at (1, 0) which is dirty, it should suck
#     ((loc_C, 'Clean'),): 'Up',
#     ((loc_C, 'Dirty'),): 'Suck',
#     ((loc_D, 'Clean'),): 'Left',
#     ((loc_D, 'Dirty'),): 'Suck',
...
#     ((loc_A, 'Dirty'), (loc_A, 'Clean'), (loc_B, 'Clean'), (loc_D, 'Clean')): 'NoOp',
...
#     ((loc_B, 'Dirty'), (loc_B, 'Clean'), (loc_D, 'Clean'), (loc_C, 'Clean')): 'NoOp',
...
#     ((loc_C, 'Dirty'), (loc_C, 'Clean'), (loc_A, 'Clean'), (loc_B, 'Clean')): 'NoOp',
...
#     ((loc_D, 'Dirty'), (loc_D, 'Clean'), (loc_C, 'Clean'), (loc_A, 'Clean')): 'NoOp',
#     ((loc_D, 'Dirty'), (loc_D, 'Clean'), (loc_C, 'Clean'), (loc_A, 'Dirty')): 'Suck',
    
#     # Sequences after cleaning a location
#     ((loc_A, 'Dirty'), (loc_A, 'Clean')): 'Right',
#     ((loc_B, 'Dirty'), (loc_B, 'Clean')): 'Down',
#     ((loc_C, 'Dirty'), (loc_C, 'Clean')): 'Up',
#     ((loc_D, 'Dirty'), (loc_D, 'Clean')): 'Left',
...
def TableDrivenVacuumAgent():
    """A vacuum agent that uses a comprehensive table to decide actions based on percept sequences in a 2x2 environment."""
    table = {
        # Single percepts
        ((loc_A, 'Clean'),): 'Right',
        ((loc_A, 'Dirty'),): 'Suck',
...
# locations for a 2x2 environment
loc_A, loc_B, loc_C, loc_D = (0, 0), (1, 0), (0, 1), (1, 1)
...
class TrivialVacuumEnvironment(Environment):
    ...
    def __init__(self):
        # inherit from environment class
        super().__init__() 
    ...
    # def add_thing(self, thing, location=None):
    #     """Add things to the world. If location is None, a random location will be chosen."""
    #     if location is None:
    ...
    # def is_inbounds(self, location):
    #     """Check if a location is inbounds (within the 2x2 grid)."""
    #     return location in [loc_A, loc_B, loc_C, loc_D]
